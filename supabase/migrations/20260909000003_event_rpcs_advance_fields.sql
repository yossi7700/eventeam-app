-- Extend create_event_with_children / update_event_with_children to accept
-- the 8 advance-setting overrides and the products.color field added in
-- 20260909000001. Recreated in full (Postgres has no ALTER FUNCTION for
-- adding parameters) -- bodies are otherwise identical to the originals in
-- 20260826000006/7 aside from the new columns.
...
create or replace function public.create_event_with_children(
  ...
  p_sub_events jsonb, -- [{ title, description, location, start_at, end_at, capacity,
                      ...
                      --    products: [{ name, description, price, currency, capacity,
                      --                 color, sort_order }] }]
  p_advance jsonb default null -- { is_attendees_required, is_show_address, is_cash_allowed,
                                --   is_donation_allowed, is_show_regulation, is_show_stripe,
                                --   is_show_app_fee, is_enable_donation } -- any key omitted
                                -- or null means "inherit from company/platform default"
)
returns uuid
language plpgsql
security definer
