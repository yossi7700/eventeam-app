...
### 3.6 גרף הכנסות/אורחים בדשבורד (חדש, החלטה שלך)
במערכת הישנה, הדשבורד כלל גרף (chart_data) של כמות אורחים/הכנסות לפי אירוע. הוספתי את שני הרכיבים האחרים שהיו חסרים (אירועים קרובים, נרשמים אחרונים), אבל **הגרף עדיין לא בנוי** — זה דורש הוספת ספריית גרפים (recharts הכי נפוץ עם React/Next.js) שלא הייתה קיימת בפרויקט, וזו החלטה נפרדת מתיקון פער ישיר.

- [ ] תגיד לי אם תרצה שאוסיף את זה (מומלץ: recharts, קל משקל ופופולרי)
