...
### 3.7 ייצוא CSV גלובלי (כל החברות ביחד, admin) — לא נבנה, בכוונה
במערכת הישנה היו `DashboardController::exportAll`/`exportAllOld` — ייצוא CSV של *כל* הלידים מכל החברות (עם פילטר `company_id=all`), אבל הם כתבו את הקובץ ישירות ל-`public_path('files/yosef-leads/...')` עם שם קובץ הניתן לניחוש — כלומר כל מי שמנחש/סורק את הנתיב יכול להוריד PII (שם/מייל/טלפון) של אורחים מכל חברה, בלי הרשאה, והקבצים גם לא נמחקים אף פעם. זה בדיוק סוג הבאג ש-`export-leads-csv` (per-event, Signed URL פרטי מ-bucket) כבר פותר; ה-CSV שלנו גם עשיר יותר (כולל breakdown לפי סוג כרטיס/כמות/מחיר).

בניתי רק ייצוא ברמת-אירוע בודד (`export-leads-csv`) — אין כרגע כפתור "ייצא הכל מכל החברות" למנהל-על. זו נראית תוספת נוחות קטנה יחסית (השם `yosef-leads` מרמז על סקריפט דיבוג אישי, לא פיצ'ר מוצרי מלוטש) ולא פונקציונליות ליבה, אז לא בניתי אותה מחדש בלי לשאול.
- [ ] תגיד לי אם אתה כן רוצה ייצוא CSV גלובלי (כל החברות) למנהל-על — קל להוסיף כ-Edge Function נוספת עם אותה תבנית Signed URL הבטוחה.
